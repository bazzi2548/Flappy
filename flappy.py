...
def main():
    ...
    SOUNDS['eat'] = pygame.mixer.Sound('assets/audio/eat' + soundExt)
    ...
    while True:
        ...
        IMAGES['jelly'] = (
            pygame.image.load(
                'assets/sprites/bonussmalljelly.png').convert_alpha()
        )
        ...
        HITMASKS['jelly'] = (getHitmask(IMAGES['jelly']))   
...
def mainGame(movementInfo):
    ...
    newJelly1 = getRandomJelly()
    newJelly2 = getRandomJelly()

    jellys = [
        {'x': SCREENWIDTH + random.randint(300, 580), 'y': newJelly1[0]['y']},
        {'x': SCREENWIDTH + random.randint(300, 580) + (SCREENWIDTH / 2), 'y': newJelly2[0]['y']},
    ]
    ...
    while True:
        ...
        checkJellyTest = checkJelly({'x': playerx, 'y': playery, 'index': playerIndex}, jellys)
        ...
        if len(upperPipes) > 0 and 0 < upperPipes[0]['x'] < 5:
            ...
            newJelly = getRandomJelly()
            jellys.append(newJelly[0])
        ...
        if jellys[0]['x'] < -IMAGES['jelly'].get_width():
            jellys.pop(0)
        ...
        for jelly in jellys:
            SCREEN.blit(IMAGES['jelly'], (jelly['x'],jelly['y']))
...
def checkJelly(player, jellys):
    ...
    jellyW = IMAGES['jelly'].get_width()
    jellyH = IMAGES['jelly'].get_height()

    for jelly in jellys:
        jellyRect = pygame.Rect(jelly['x'], jelly['y'], jellyW, jellyH)

        pHitMask = HITMASKS['player'][pi]
        jHitMask = HITMASKS['jelly']

        jCollide = pixelCollision(playerRect, jellyRect, pHitMask, jHitMask)
